# Define a HeteroData object given a graph with k-node-tuples and a collection of adjacency types.
...
from torch_geometric.data.custom_complex import KNodeTuple
from torch_geometric.transforms.add_metapaths_hops import AddMetaPathsHops


class KTupleWiring(object):
    def __init__(self, adjacency_types: List[str] = ["boundary", "upper"]):
        self.adjacency_types = adjacency_types
        super().__init__()

    def __call__(self, data: KNodeTuple, boundary_adjacency_tensors: List[SparseTensor]) -> HeteroData:
        ...
        for d in range(het_data.dim):
            if "boundary" not in self.adjacency_types:
                ...
                # boundaries can be derived from node-tuple indices, no need to compute them.
                boundaries, cells = torch.tensor(data=[], dtype=torch.long), torch.tensor(data=[],
                                                                                          dtype=torch.long)  # TODO: specify
        ...
        if "upper" in self.adjacency_types:
            for d in range(het_data.dim):
                metapaths.append([(f"{d}_cell", "boundary_of", f"{d + 1}_cell"),
                                  (f"{d + 1}_cell", "coboundary_of", f"{d}_cell")])
        if "lower" in self.adjacency_types:
            for d in range(het_data.dim):
                metapaths.append([(f"{d + 1}_cell", "coboundary_of", f"{d}_cell"),
                                  (f"{d}_cell", "boundary_of", f"{d + 1}_cell")])
