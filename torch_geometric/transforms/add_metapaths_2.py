...
@functional_transform('add_metapaths_2')
class AddMetaPaths2(BaseTransform):
    ...
    def __call__(self, data: HeteroData) -> HeteroData:
        ...
        for j, metapath in enumerate(self.metapaths):
            ...
            metapath[0][0], f'metapath_{j}_st',metapath[-1][-1]) # source_to_target

            assert(len(row)==len(col))
            # Only a self-edges cause errors --> TODO
            if(len(row)>1):
